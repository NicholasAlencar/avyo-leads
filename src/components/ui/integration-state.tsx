  ...
  return (
    <article aria-label={status.label} className="rounded-[var(--radius-panel)] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow-card)]" data-state={status.configured ? "configured" : "unconfigured"}>
      <div className="flex items-center gap-3">
        <span className={`grid size-10 shrink-0 place-items-center rounded-xl ${status.configured ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-700"}`}>{status.configured ? <Check className="size-5" /> : <Settings2 className="size-5" />}</span>
        ...
        <span
          className={
            status.configured
              ...
              : "rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold tracking-wide text-amber-800"
          }
        >
