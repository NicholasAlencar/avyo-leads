  ...
  return (
    <section className="mx-auto max-w-[1500px]">
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-700">Ficha consolidada</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">{lead.companyName}</h1><p className="mt-2 text-sm text-slate-500">{pipelineLabels[lead.pipelineStage]}</p></div>
      ...
      <div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="space-y-5"><LeadOverview lead={lead} workspace={workspace} /><LeadTimeline activities={workspace.activities} /></div><div className="space-y-5"><section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="text-sm font-bold">Etapa comercial</h2><StageControl leadId={lead.id} stage={lead.pipelineStage} /></section><FollowupForm leadId={lead.id} members={workspace.members} defaultOwner={lead.ownerId ?? member.userId} /><LeadRelations lead={lead} workspace={workspace} /></div></div>
    </section>
  );
}
