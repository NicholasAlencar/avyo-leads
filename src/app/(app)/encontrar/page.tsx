...
export default async function FindCompaniesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="space-y-6">
      <div><p className="text-sm font-semibold text-blue-600">PROSPECÇÃO</p><h1 className="text-4xl font-semibold tracking-[-.04em] text-slate-950">Encontre a próxima oportunidade.</h1><p className="mt-1 text-slate-600">Pesquisa oficial no Google Places. Nenhum resultado é inventado.</p></div>
      <form className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-[var(--avyo-shadow)] md:grid-cols-3">
        ...
        <button className="rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-3 font-bold text-white shadow-lg shadow-blue-500/20 md:col-span-3" type="submit">BUSCAR LEADS</button>
      </form>
      ...
      {result?.ok && searchData && <section className="space-y-3"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">{result.data.length} empresas encontradas</h2><span className="text-xs text-slate-500">Fonte: Google Places</span></div>{result.data.map((place) => <article className="rounded-2xl border border-slate-200 bg-white p-5" key={place.placeId}><div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-lg font-bold">{place.companyName}</h3><p className="text-sm text-slate-600">{place.category ?? "Categoria não informada"} · {place.address ?? "Endereço não informado"}</p><p className="mt-2 text-sm">{place.rating ? `${place.rating.toFixed(1)} (${place.reviewCount ?? 0} avaliações)` : "Sem avaliação disponível"} {place.phone ? ` · ${place.phone}` : ""}</p></div><div className="flex gap-2">{place.googleMapsUrl && <a className="rounded-lg border px-3 py-2 text-sm" href={place.googleMapsUrl} rel="noreferrer" target="_blank">Maps</a>}<Link className="rounded-lg bg-blue-600 text-white px-3 py-2 text-sm font-bold" href={`/leads/novo?companyName=${encodeURIComponent(place.companyName)}&segment=${encodeURIComponent(place.category ?? searchData.segment)}&city=${encodeURIComponent(place.city ?? searchData.city)}&stateCode=${encodeURIComponent(place.stateCode ?? "")}&phone=${encodeURIComponent(place.phone ?? "")}&websiteUrl=${encodeURIComponent(place.websiteUrl ?? "")}&googleMapsUrl=${encodeURIComponent(place.googleMapsUrl ?? "")}&googlePlaceId=${encodeURIComponent(place.placeId)}`}>Revisar e importar</Link></div></div></article>)}</section>}
    </main>
  );
}
